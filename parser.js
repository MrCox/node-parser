...
var parser = chain(function(from, to) {
  ...
  self.var('headers', null, function(_) {
    if (typeof _ === "number") self._vars.headers = u.range(_);
  });
  ...
  self.each = function(cb) { return _queue.push(cb) && self; };
  ...
  self.batch = function(dirs, key) {
    var methods = u.keys(self), parsers;
    methods.push('on');

    if (typeof dirs === "string") {
      var dir = fs.readdirSync(dirs);
      parsers = dir.map(function(d, i) {
        return parser(path.join(dirs, d));
      });
      dirs = dir;
    } else parsers = dirs.map(function(d) { return parser(d); });

    var data = key ? dirs.map(key) : dirs;
    return chain = u.chain(parsers, methods, true, ex).data(data);
    function ex(chain) {
      chain.execute = function(fn) {
        var data = chain.data();
        chain.forEach(function(obj, i) {
          obj.execute(function() { fn && fn.call(obj, data[i]); });
        });
        return chain;
      };
      chain.reduce = function(fn, func, init) {
        if (typeof fn === "string")
          return chain.method('reduce', fn, func, init);

        var data = chain.data();
        chain.forEach(function(obj, i) {
          obj.reduce(fn(data[i], i), func, init);
        });
        return chain;
      };
      var on = chain.on;
      chain.on = function(event, cb) {
        if (event == "all_finish") {
          var count = 0;
          chain.forEach(function(obj) {
            obj.on('finish', function() { if (++count == chain.length) cb(); });
          });
        } else on(event, cb);
        return chain;
      };
